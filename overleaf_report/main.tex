...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{TTL:} \texttt{default\_time\_to\_live = 15552000} (180 ngày). Đảm bảo dữ liệu tồn tại đủ lâu để Cold Archiver (6 tháng) kịp sao lưu sang Parquet.
...
% ============================================================
% PHASE 4: PHÂN TÍCH NLP
% ============================================================
\newpage
\section{Phase 4: Phân tích Cảm xúc và Xử lý Ngôn ngữ Tự nhiên (NLP)}
\label{sec:phase4}

\subsection{Mục tiêu}
Làm sạch dữ liệu văn bản từ hệ thống chat (loại bỏ ký tự đặc biệt, URL, stopwords) và chuẩn bị dữ liệu thô phục vụ cho các bài toán phân tích ngôn ngữ tự nhiên (NLP) ở các giai đoạn sau.

\subsection{Tiền xử lý văn bản (Text Preprocessing)}
Quy trình làm sạch dữ liệu bao gồm 5 bước tiêu chuẩn:
\begin{itemize}[noitemsep]
    \item \textbf{Chuyển đổi chữ thường:} Đưa toàn bộ văn bản về lowercase.
    \item \textbf{Loại bỏ URL:} Sử dụng Regex để lọc bỏ các liên kết web.
    \item \textbf{Bảo tồn Emoticon:} Các biểu tượng cảm xúc mang nhiều ý nghĩa (vd: \texttt{=)))}, \texttt{<3}, \texttt{!!!}, \texttt{???}) được tách riêng để không bị xóa bởi bộ lọc dấu câu.
    \item \textbf{Tách từ (Tokenize):} Sử dụng thư viện \texttt{underthesea} để xử lý segmentation tiếng Việt chính xác.
    \item \textbf{Loại bỏ Stopwords:} Dùng danh sách stopwords tùy chỉnh tiếng Việt và tiếng Anh.
\end{itemize}
...
% ============================================================
\newpage
\section{Phase 5: Backend Simulation \& Dashboard Streamlit}
\label{sec:phase5}

\subsection{Mục tiêu}
Xây dựng giao diện trực quan hóa dữ liệu bằng Streamlit và mô phỏng truy vấn backend lấy tin nhắn theo phân trang (pagination) và lùi tháng (backtracking).

\subsection{Biểu đồ phân tích NLP}
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{wordcloud_room_999.png}
        \caption{Đám mây từ vựng tiếng Việt.}
        \label{fig:wordcloud}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{sentiment_trend.png}
        \caption{Xu hướng cảm xúc theo thời gian.}
        \label{fig:sentiment}
    \end{subfigure}
    \caption{Phân tích ngôn ngữ tự nhiên từ nội dung chat.}
    \label{fig:nlp_charts}
\end{figure}
...
\midrule
Phase 0 & Thiết lập Docker Compose, sinh Mock Data & Hoàn thành (100\%) \\
Phase 1 & Phân tích EDA, phát hiện Hot Partition & Hoàn thành (100\%) \\
Phase 2 & Thiết kế schema ScyllaDB, Time-bucketing & Hoàn thành (100\%) \\
Phase 3 & Pipeline ETL, Tối ưu I/O, Cold Archiver & Hoàn thành (100\%) \\
Phase 4 & Tiền xử lý văn bản tiếng Việt \& NLP & Hoàn thành (100\%) \\
Phase 5 & Backend Simulation \& Dashboard Streamlit & Hoàn thành (100\%) \\
\bottomrule
\end{tabular}
\end{table}
...
Python & 3.x (Scripting, ETL, NLP) \\
cassandra-driver & Kết nối Python $\leftrightarrow$ Cassandra/ScyllaDB \\
Matplotlib & Trực quan hóa dữ liệu \\
underthesea & Tiền xử lý ngôn ngữ tiếng Việt (Tokenize) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[noitemsep]
    \item \textbf{Phân tích Cảm xúc (Sentiment Analysis):} Áp dụng Dictionary-based hoặc PhoBERT để phân loại cảm xúc tin nhắn.
    \item \textbf{Trực quan hóa Dữ liệu (WordCloud):} Vẽ đám mây từ khóa thể hiện xu hướng thảo luận trong các phòng chat.
    \item \textbf{Xử lý luồng (Streaming Processing):} Tích hợp Apache Kafka và Spark Streaming để phân tích cảm xúc tin nhắn theo thời gian thực (Real-time).
    \item \textbf{Giám sát Production:} Tích hợp hệ thống monitoring (Prometheus/Grafana) cho cụm cơ sở dữ liệu ScyllaDB.
\end{enumerate}
